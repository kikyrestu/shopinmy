        ...
        <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 sm:p-8">
            ...
            @elseif($error === 'waiting_update')
                <div class="bg-blue-50/50 border border-blue-100 p-8 rounded-3xl text-center max-w-xl mx-auto mt-4 mb-4">
                    <div class="w-20 h-20 bg-white rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm ring-4 ring-blue-50">
                        <i class="ph-duotone ph-truck text-4xl text-blue-500 animate-pulse"></i>
                    </div>
                    <h3 class="text-xl font-bold text-gray-900 mb-2">{{ __('Menunggu Update Kurir') }}</h3>
                    ...
                    @if($order->tracking_no && !str_starts_with($order->tracking_no, 'ORD-'))
                        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm mb-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-left">
                            <div>
                                <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">{{ __('Kurir') }}</span>
                                <span class="block text-sm font-bold text-gray-900 uppercase mt-0.5">{{ $order->courier ?? 'Unknown' }}</span>
                            </div>
                            <div class="w-px h-10 bg-gray-100 hidden sm:block"></div>
                            <div class="flex-1 text-center sm:text-left">
                                <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">{{ __('Nomor Resi') }}</span>
                            ...
                            <div x-data="{ copied: false }">
                                <button @click="navigator.clipboard.writeText('{{ $order->tracking_no }}'); copied = true; setTimeout(() => copied = false, 2000)" 
                                        class="flex items-center gap-2 px-4 py-2 bg-gray-50 hover:bg-gray-100 text-gray-600 rounded-xl transition-colors text-sm font-semibold">

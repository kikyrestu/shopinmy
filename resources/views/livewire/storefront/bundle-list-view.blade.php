    ...
    <main class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-8 -mt-6 relative z-10">
        @if($bundles->isEmpty())
            <div class="bg-white rounded-3xl p-12 text-center shadow-sm border border-gray-100">
                ...
                <h2 class="text-xl font-bold text-gray-900">{{ __('No bundles available right now') }}</h2>
                <p class="text-gray-500 mt-2">{{ __('Check back later for exciting combo deals!') }}</p>
            </div>
        @else
            <div class="space-y-8">
                @foreach($bundles as $bundle)
                    <div class="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 flex flex-col md:flex-row gap-8">
                        
                        <!-- Bundle Details & Products -->
                        <div class="flex-1">
                            <h2 class="text-2xl font-bold text-gray-900 mb-2">{{ $bundle->name }}</h2>
                            @if($bundle->description)
                                <p class="text-gray-500 mb-6">{{ $bundle->description }}</p>
                            @endif
                            
                            <div class="space-y-4">
                                <h3 class="text-sm font-bold text-gray-400 uppercase tracking-wider">{{ __('Includes') }}:</h3>
                                    ...
                                    @foreach($bundle->products as $product)
                                        ...
                                        <div class="flex items-center gap-3 p-3 rounded-2xl border border-gray-100 bg-gray-50">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-500">{{ $product->pivot->qty }}x</p>
                                                <p class="text-xs font-bold text-gray-400 line-through mt-1">RM {{ number_format($product->price, 2) }}</p>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="w-full md:w-80 bg-amber-50 rounded-2xl p-6 border border-amber-100 flex flex-col justify-center relative overflow-hidden">
                            ...
                            <div class="relative z-10">
                                <div class="mb-4">
                                    <p class="text-sm font-bold text-gray-500 mb-1">{{ __('Bundle Price') }}</p>
                                    <div class="flex items-baseline gap-2">
                                        <span class="text-3xl font-black text-amber-600">RM {{ number_format($bundle->price, 2) }}</span>
                                    </div>
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="text-sm font-bold text-gray-400 line-through">RM {{ number_format($totalOriginalPrice, 2) }}</span>

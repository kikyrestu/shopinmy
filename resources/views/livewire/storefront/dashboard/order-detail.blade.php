<div>
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            <a href="{{ route('dashboard.orders') }}" class="w-10 h-10 bg-white border border-gray-200 rounded-xl flex items-center justify-center text-gray-500 hover:text-brand-600 hover:border-brand-200 hover:bg-brand-50 transition-all shadow-sm">
                <i class="ph-bold ph-arrow-left text-lg"></i>
            </a>
            <h1 class="text-2xl font-extrabold text-gray-900">{{ __('Order Details') }}</h1>
        </div>
                ...
                <div class="divide-y divide-gray-50">
                    @foreach($order->items as $item)
                        <div class="p-6 flex flex-col sm:flex-row gap-5 items-start sm:items-center">
                            <div class="w-20 h-20 bg-gray-50 rounded-2xl border border-gray-100 overflow-hidden flex-shrink-0">
                            ...
                            <div class="flex-1 min-w-0">
                                <h4 class="text-base font-bold text-gray-900 truncate mb-1">{{ $item->product->name }}</h4>
                                <div class="text-sm text-gray-500 flex flex-wrap items-center gap-x-4 gap-y-2">
                                    ...
                                    @if($item->variant)
                                        <span class="px-2 py-0.5 bg-gray-100 rounded-md text-xs">{{ $item->variant->name }}: {{ $item->variant->value }}</span>
                                    @endif
                                </div>
                            </div>
                            <div class="text-right w-full sm:w-auto mt-2 sm:mt-0">
                                <div class="text-sm text-gray-500 mb-0.5">RM {{ number_format($item->price, 2) }} / {{ __('item') }}</div>
                                <div class="text-lg font-extrabold text-gray-900">RM {{ number_format($item->price * $item->qty, 2) }}</div>
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 sm:p-8">
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-center text-gray-600">
                        ...
                        <span class="font-bold text-gray-900">RM {{ number_format($order->total - $order->shipping_cost - $order->tax_amount + ($order->voucher_id ? (\App\Models\Voucher::find($order->voucher_id)?->value ?? 0) : 0), 2) }}</span>
                    </div>
                    
                    <div class="flex justify-between items-center text-gray-600">
                        <span>{{ __('Shipping Cost') }}</span>
                        <span class="font-bold text-gray-900">RM {{ number_format($order->shipping_cost, 2) }}</span>
                    </div>

                    <div class="flex justify-between items-center text-gray-600">
                        <span>{{ __('Tax') }}</span>
                        <span class="font-bold text-gray-900">RM {{ number_format($order->tax_amount, 2) }}</span>
                    </div>
                    ...
                    <div class="pt-4 border-t border-gray-100 flex justify-between items-center">
                        <span class="text-lg font-bold text-gray-900">{{ __('Grand Total') }}</span>
        ...
        <div class="space-y-6">
            ...
            @if($order->tracking_no && !str_starts_with($order->tracking_no, 'ORD-'))
                <div class="bg-brand-50 border border-brand-200 rounded-3xl p-6 shadow-sm relative overflow-hidden">
                    ...
                    <div class="font-bold text-gray-900 text-xl mb-4 relative z-10">{{ $order->tracking_no }}</div>
            ...
            <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
                <h2 class="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4">{{ __('Order Info') }}</h2>
                <div class="space-y-4">
                    <div>
                        <div class="text-xs text-gray-500 mb-1">{{ __('Order Number') }}</div>
                        <div class="font-bold text-gray-900">{{ $order->order_number }}</div>
                    </div>
                    <div>
                        <div class="text-xs text-gray-500 mb-1">{{ __('Order Date') }}</div>
            ...
            <!-- Shipping Details -->
            <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
                <h2 class="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4">{{ __('Shipping Details') }}</h2>
                
                <div class="mb-4">
                    <div class="text-xs text-gray-500 mb-1">{{ __('Courier') }}</div>
                ...
                <div>
                    <div class="text-xs text-gray-500 mb-1">{{ __('Shipping Address') }}</div>
            ...
            <!-- Payment Details -->
            <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
                <h2 class="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4">{{ __('Payment Details') }}</h2>
                <div class="space-y-4">
                    <div>
                        <div class="text-xs text-gray-500 mb-1">{{ __('Method') }}</div>
                    ...
                    <div>
                        <div class="text-xs text-gray-500 mb-1">{{ __('Payment Status') }}</div>

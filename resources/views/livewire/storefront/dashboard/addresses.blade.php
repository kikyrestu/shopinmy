<div>
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <h1 class="text-2xl font-extrabold text-gray-900">{{ __('Address Book') }}</h1>
    ...
    @if($isEditing)
        <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 sm:p-8 mb-8 animate-fade-in-up">
            <h2 class="text-lg font-bold text-gray-900 mb-6">{{ $addressId ? __('Edit Address') : __('Add New Address') }}</h2>
            <form wire:submit.prevent="saveAddress">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                    <div class="sm:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('Label (e.g. Home, Office)') }}</label>
                        <input type="text" wire:model="label" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" wire:model="address" required class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all">
                    ...
                    <div>
                        ...
                        <input type="text" wire:model="postcode" required class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all">
                    ...
                    <div class="sm:col-span-2 mt-2">
                        <label class="flex items-center gap-3 cursor-pointer">
                            ...
                            <span class="text-sm font-medium text-gray-700">{{ __('Set as Default Address') }}</span>
                        </label>
                    </div>
                </div>
                <div class="flex justify-end gap-3">
                    <button type="button" wire:click="resetForm" class="px-6 py-2.5 text-gray-600 font-bold hover:bg-gray-100 rounded-xl transition-colors">{{ __('Cancel') }}</button>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="bg-white rounded-3xl border {{ $addr->is_default ? 'border-brand-500 ring-1 ring-brand-500' : 'border-gray-100' }} shadow-sm p-6 relative">
            @if($addr->is_default)
            ...
            <div class="flex items-start justify-between mb-4">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="font-bold text-gray-900">{{ $addr->label ?? __('Address') }}</h3>
                </div>
                <div class="flex items-center gap-1">
                    <button wire:click="editAddress({{ $addr->id }})" class="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-brand-600 hover:bg-brand-50 rounded-full transition-colors" title="{{ __('Edit') }}">
                        <i class="ph ph-pencil-simple text-lg"></i>
                    </button>
                    <button wire:click="deleteAddress({{ $addr->id }})" wire:confirm="{{ __('Are you sure you want to delete this address?') }}" class="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors" title="{{ __('Delete') }}">
        ...
        @empty
            <div class="col-span-full p-12 text-center bg-white rounded-3xl border border-gray-100">
                ...
                <h3 class="text-lg font-bold text-gray-900 mb-1">{{ __('No addresses saved') }}</h3>
                <p class="text-gray-500">{{ __('Add a shipping address to make checkout faster.') }}</p>
            </div>
        @endforelse
    </div>

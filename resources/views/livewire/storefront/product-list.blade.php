<div x-data="{ showMobileFilters: false }">
    ...
    <div class="mb-8 relative">
        ...
        <p class="text-gray-500 mt-2 text-sm">{{ __('Showing') }} {{ $products->firstItem() ?? 0 }}-{{ $products->lastItem() ?? 0 }} {{ __('of') }} {{ $products->total() }} {{ __('products') }}</p>
    </div>
    ...
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <aside class="w-full lg:w-64 flex-shrink-0 z-50 lg:z-auto" :class="showMobileFilters ? 'fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-end' : 'hidden lg:block'">
            <div class="bg-white lg:rounded-2xl lg:border border-gray-100 p-5 lg:sticky top-28 shadow-sm h-full lg:h-auto overflow-y-auto w-4/5 lg:w-full" @click.stop>
                ...
                <div>
                    <!-- Category Filter -->
                    <div class="mb-6">
                        <h3 class="text-sm font-bold text-gray-900 mb-3">{{ __('Categories') }}</h3>
                        <div class="space-y-2 max-h-48 overflow-y-auto hide-scrollbar">
                            ...
                            <label class="flex items-center gap-3 cursor-pointer group">
                                ...
                                <span class="text-sm text-gray-600 group-hover:text-brand-600 transition-colors {{ $category === $cat->slug ? 'font-semibold text-brand-600' : '' }}">{{ $cat->name }}</span>
                            </label>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <!-- Brand Filter -->
                    <div class="mb-6">
                        <h3 class="text-sm font-bold text-gray-900 mb-3">{{ __('Brands') }}</h3>
                        <div class="space-y-2 max-h-48 overflow-y-auto hide-scrollbar">
                            @foreach($brands as $b)
                            <label class="flex items-center gap-3 cursor-pointer group">
                                ...
                                <span class="text-sm text-gray-600 group-hover:text-brand-600 transition-colors">{{ $b->name }}</span>
                            </label>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <!-- Price Filter -->
                    <div class="mb-6">
                        <h3 class="text-sm font-bold text-gray-900 mb-3">{{ __('Price Range') }}</h3>
                        <div class="flex items-center gap-2">
                            <div class="relative flex-1">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs font-semibold">RM</span>
                                <input type="number" wire:model.live.debounce.500ms="min_price" placeholder="{{ __('Min') }}" class="w-full pl-9 pr-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-brand-500 focus:border-brand-500 outline-none transition-all">
                            </div>
                            <span class="text-gray-400">-</span>
                            <div class="relative flex-1">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs font-semibold">RM</span>
                                <input type="number" wire:model.live.debounce.500ms="max_price" placeholder="{{ __('Max') }}" class="w-full pl-9 pr-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-brand-500 focus:border-brand-500 outline-none transition-all">
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="flex-1 relative">
            ...
            <div wire:loading.flex class="absolute inset-0 bg-white/50 backdrop-blur-sm z-10 items-center justify-center rounded-2xl hidden">
                <div class="bg-white px-6 py-3 rounded-full shadow-lg flex items-center gap-3 font-semibold text-brand-600">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 p-4 mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <button wire:click="setSort('{{ $key }}')" 
                       class="px-4 py-2 text-sm font-semibold rounded-full transition-all border {{ $sort === $key ? 'bg-brand-50 border-brand-200 text-brand-600' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50' }}">
                        {{ $label }}
                    </button>
                    @endforeach
                </div>
            ...
            @else
                <!-- Empty State -->
                <div class="bg-white rounded-3xl border border-gray-100 p-12 flex flex-col items-center justify-center text-center shadow-sm">
                    ...
                    <p class="text-gray-500 mb-6 max-w-sm">{{ __('We could not find any products matching your current filters or search query.') }}</p>
